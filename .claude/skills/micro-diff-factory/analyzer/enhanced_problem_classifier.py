...
class VariableDependency:
    ...
    source: str
    target: str
    dependency_type: str  # "direct", "inverse", "partial", "functional"
...
class Constraint:
    """约束条件"""
    name: str
    type: str  # "hard", "soft", "boundary", "propagation"
    description: str
...
class DecompositionResult:
    ...
    variables: Dict[str, Dict]  # 变量名 -> {属性字典}
...
class EnhancedProblemClassifier:
    ...
    def __init__(self):
        ...
        self.domain_keywords = {
            ...
            ProblemDomain.UI_UX: {
                "primary": ["ui", "interface", "界面", "design", "设计", "layout", "布局", "component", "组件", "widget", "控件"],
            ...
            ProblemDomain.SECURITY: {
                ...
                "secondary": ["jwt", "token", "session", "会话", "csrf", "xss", "injection", "注入"],
    ...
    def _classify_domain(self, description: str) -> Tuple[ProblemDomain, float]:
        ...
        for domain, keywords in self.domain_keywords.items():
            # 主关键词权重 3.0
            primary_score = sum(3.0 for kw in keywords["primary"] if kw in description_lower)
            # 次关键词权重 1.5
            secondary_score = sum(1.5 for kw in keywords["secondary"] if kw in description_lower)
            # 第三关键词权重 0.5
            tertiary_score = sum(0.5 for kw in keywords["tertiary"] if kw in description_lower)
    ...
    def _determine_complexity(self, description: str, domain: ProblemDomain) -> ProblemComplexity:
        ...
        complexity_keywords = {
            ...
            ProblemComplexity.EXPERT: ["专家", "expert", "expert", "极难", "extremely hard", "extremely difficult", "最难"]
        }
    ...
    def _calculate_confidence(self, domain_confidence: float, variable_count: int, constraint_count: int) -> float:
        ...
        variable_bonus = min(0.3, variable_count / 10)  # 最多 10 个变量加满 0.3
        constraint_bonus = min(0.3, constraint_count / 5)  # 最多 5 个约束加满 0.3
...
"""
        ...
        for i, constraint in enumerate(result.constraints, 1):
            report += f"{i}. **{constraint.name}** ({constraint.type})\n"
...
识别到 {len(result.variable_dependencies)} 个依赖关系：
...
if __name__ == "__main__":
    ...
    result1 = classifier.classify_problem(
        "蓝牙连接不稳定，经常断开，需要实现指数退避重连策略，并考虑 RSSI 信号质量进行自适应调整"
    )
