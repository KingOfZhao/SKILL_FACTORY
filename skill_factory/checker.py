...
class ValidationResult:
    ...
    severity: str
    message: str
    location: str | None = None  # 文件路径、行号等
...
class MetaSkillChecker:
    ...
    def validate_skill(
        self, skill_path: str, skill_content: str | None = None
        ...
        overall_score = (
            best_practices_compliance * 0.4 + pattern_usage_score * 0.3 + structural_pass_rate * 0.3
        )
    ...
    def _validate_structure(
        self, rule: ValidationRule, content: str, path: str
    ...
    def _generate_recommendations(
        self, results: list[ValidationResult], skill_name: str
    ) -> list[str]:
        """生成推荐"""
        recommendations = []
...
def run(path: str, summary: bool = False, json_out: str | None = None) -> int:
    """执行校验；供 CLI 与薄壳共用。存在 critical/major 失败返回 1。"""
    checker = MetaSkillChecker({})

    if os.path.isdir(path):
        reports = checker.validate_multiple_skills(path)
    else:
        reports = [checker.validate_skill(path)]
    ...
    if summary or len(reports) > 1:
        print(checker.generate_validation_summary(reports))

    # 退出码：存在 critical/major 失败 → 1
    has_blocking = any(
        x.severity in ("critical", "major") for r in reports for x in r.results if not x.passed
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run(path=args.path, summary=args.summary, json_out=args.json_out)
