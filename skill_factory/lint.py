...
按 ``meta-flutter-impl-distillation`` 的 anti-patterns.md 中的高频反模式，对 Dart
源码做轻量静态扫描，输出结构化结果（JSON 或文本），并用退出码反映严重程度：

- ``0`` 无 major/critical 命中
- ``1`` 存在 major/critical 命中
- ``2`` 用法错误

设计原则：仅依赖标准库；启发式检测，宁可漏报不过度误报；AP 编号与 anti-patterns.md 一一对应。
"""
...
LineRule = tuple[str, str, "re.Pattern[str]", str, str, "re.Pattern[str] | None"]
...
def _find_build_body(src: str) -> list[tuple[int, str]]:
...
def scan_text(content: str, path: str) -> list[Finding]:
    ...
    for idx, raw in enumerate(lines, start=1):
        ...
        for ap_id, sev, pat, msg, fix, exclude in LINE_RULES:
            if pat.search(line) and not (exclude and exclude.search(line)):
                findings.append(Finding(ap_id, sev, msg, path, idx, line.strip()[:160], fix))
        # AP-4 滥用 null 断言
        if _scan_bang(line):
            findings.append(
                Finding(
    ...
    creates_ctrl = re.search(
        r"(TextEditingController|AnimationController|ScrollController)\s*\(", src
    )
...
def run(
    ...
    if selftest:
        return run_selftest()
    if not path:
        print("错误：需要提供 path 或 --selftest", flush=True)
        return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run(path=args.path, fmt=args.format, json_out=args.json_out, selftest=args.selftest)
